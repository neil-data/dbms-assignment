<?php
/**
 * CEMS - Leave Club Endpoint
 * POST /backend/clubs/leave.php
 * Handles student leaving an enrolled club. Requires real student session.
 */
...
try {
    ...
    sendSuccess("You have left {$club['club_name']}.", [
        'club_id'      => $clubId,
    ...
    error_log('Database error in clubs/leave.php: ' . $e->getMessage());
    sendError('Database error occurred while processing club departure.', [], 500);
} catch (Exception $e) {
    error_log('General error in clubs/leave.php: ' . $e->getMessage());
